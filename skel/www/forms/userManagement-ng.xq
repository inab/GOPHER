...
return if(empty($dumpost)) then (
	...
	<head>
		...
		<xforms:model id="default-model">
			...
			<xforms:bind nodeset="instance('users')" constraint="count(xcesc:user) eq count(distinct-values(xcesc:user/@nickname/string()))">
	...
	<body>
		...
		<xforms:repeat nodeset="xcesc:user" id="usuarios" appearance="compact">
			...
			<xforms:trigger class="delete">
				<xforms:label>X</xforms:label>
				<xforms:action ev:event="DOMActivate">
					<xforms:insert nodeset="/xcesc:users/xcesc:deletedUser" position="after" at="last()"/>
					<xforms:setvalue ref="/xcesc:users/xcesc:deletedUser[last()]/@nickname" value="@nickname"/>
					<xforms:setvalue ref="/xcesc:users/xcesc:deletedUser[last()]/@id" value="@id"/>
